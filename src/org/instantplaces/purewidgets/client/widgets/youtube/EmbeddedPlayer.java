...
public class EmbeddedPlayer extends Widget {
	...
	private static final Map<String, EmbeddedPlayer> map = new HashMap<String, EmbeddedPlayer>();
	...
	public EmbeddedPlayer(String name, PlayerListener listener) {
		this(name, "640", "480", false, listener);
	}

	public EmbeddedPlayer(String name, boolean listenLoading,
			PlayerListener listener) {
		this(name, "640", "480", listenLoading, listener);
	}

	public EmbeddedPlayer(String name, String width, String height,
			PlayerListener listener) {
	...
	public void preLoadVideoById(String videoId) {
		...
		if (this.listenLoading) {
			...
			loaderTimer = new Timer() {
				...
				@Override
				public void run() {
					int loaded = EmbeddedPlayer.this.getVideoBytesLoaded();
					int total = EmbeddedPlayer.this.getVideoBytesTotal();
					if (loaded != -1) {
						...
						if (loaded >= total) {
							...
							int toWait = map(0, 100,
									EmbeddedPlayer.MAX_LOADING_INTERVAL,
									EmbeddedPlayer.MIN_LOADING_INTERVAL,
									loadedPercent);
							this.schedule(toWait);
							Log.debug("Waiting " + toWait + " seconds");
							for (PlayerListener pl : EmbeddedPlayer.this.listener) {
						...
						if (minuscount > 6) {
							EmbeddedPlayer.this
									.preLoadVideoById(EmbeddedPlayer.this.currentVideoID);
							Log
	...
	private static void onError(String playerId, int error) {
		// Log.debug("OnStateChange " + playerId + " " + state);
		EmbeddedPlayer player = map.get(playerId);
		if (player != null && player.listener != null ) {
			PlayerError pError = null;
			switch ( error ) {
			case 2:
				pError = PlayerError.INVALID_PARAMETER;
				break;
			case 100:
				pError = PlayerError.NOT_FOUND;
				break;
			case 101:
			case 150:
				pError = PlayerError.NO_EMBED;
				break;
			}
			for (PlayerListener pl : player.listener) {
				/*
	...
	static {
		register();
		Log.debug("EmbeddedPlayer", "Called register");
	}
	...
	private static native void register() /*-{
		$wnd.onYouTubePlayerReady = function(playerId) {
		      ...
		      @org.instantplaces.purewidgets.client.widgets.youtube.EmbeddedPlayer::onReady(Ljava/lang/String;Lorg/instantplaces/purewidgets/client/widgets/youtube/EmbeddedPlayer$YTPlayer;)(playerId, ytplayer);

		      var name = 'onPlayerStateChange'+playerId;

		      $wnd[name]  = function(state) {	      		  
		              @org.instantplaces.purewidgets.client.widgets.youtube.EmbeddedPlayer::onStateChange(Ljava/lang/String;I)(playerId, state);
		      }
		      ...
		      $wnd[nameError]  = function(error) {	      		  
		              @org.instantplaces.purewidgets.client.widgets.youtube.EmbeddedPlayer::onError(Ljava/lang/String;I)(playerId, error);
		      }
	...
	public static class YTPlayer extends JavaScriptObject {
		...
		public native final void createSwf(String name, String width, String height) /*-{
			...
			//   name, ""+width, ""+height, "8", null, null, params, atts);
			  $wnd.swfobject.embedSWF("http://www.youtube.com/v/u1zgFlCw8Aw?enablejsapi=1&version=3&playerapiid=" + name,
			   name, width, height, "8", null, null, params, atts);
		}-*/;
	}
