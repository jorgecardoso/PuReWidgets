...
public class WidgetManager  {
	 ...
	 */
	public WidgetManager(String placeId, String applicationId, 
			LocalStorage localStorage, InteractionManager interactionManager) {
		...
		this.communicator.createChannel(this.placeId, this.applicationId, this.applicationId, new AsyncCallback<ArrayList<WidgetInput>>(){

			@Override
			public void onFailure(Throwable caught) {
				Log.debug(WidgetManager.this, "Could not create input channel. Going to use polling insted");
				WidgetManager.this.timerInput.schedule(WidgetManager.this.currentInputRequestInterval);
			}
	...
	public void addWidget(Widget widget) {
		...
		if (-1 == index) {
			...
			Log.warn(this, "Widget '" + widget.getWidgetId() + "' already exists in widget list, replacing entry " + index + ".");
		...
		if (-1 == index) {
			...
			Log.warn(this, "Widget '" + widget.getWidgetId() + "' already exists in to add widget list, replacing entry " + index + ".");
	...
	public void removeWidget(Widget widget) {
		...
		if (-1 == index) {
			...
			Log.warn(this, "Widget '" + widget.getWidgetId() + "' already exists in to delete widget list, replacing entry " + index + ".");
	...
	private void periodicallySendWidgetsToServer() {	
		...
		if ( NextWidgetAction.ADD == this.nextWidgetAction ) {
			...
			if (this.toAddWidgetPool.size() > 0) {
				 ...
				 */
				this.communicator.addWidgetToServer(this.placeId, this.applicationId, this.applicationId, 
						this.toAddWidgetPool, new AsyncCallback<ArrayList<Widget>>(){

							@Override
							public void onFailure(Throwable caught) {
								WidgetManager.this.currentWidgetRequestInterval *= 2;
								timerWidget.schedule(WidgetManager.this.currentWidgetRequestInterval);
								Log.debugFinest(this, "Scheduling next widget request in " + (WidgetManager.this.currentWidgetRequestInterval/1000) +" seconds");
							}
		...
		if ( NextWidgetAction.DELETE == this.nextWidgetAction ) {
			if (this.toDeleteWidgetPool.size() > 0) {
				 ...
				 * Send the current set of widgets to the server.
				 */
				this.communicator.removeWidget(this.placeId, this.applicationId, this.applicationId, 
						this.toDeleteWidgetPool, new AsyncCallback<ArrayList<Widget>>(){
	...
	private void periodicallyAskForInputFromServer() {
		...
		this.communicator.getInputFromServer(this.placeId, this.applicationId, this.applicationId, 
			...
			public void onFailure(Throwable caught) {
				...
				Log.debugFinest(this, "Scheduling next input request in " + (WidgetManager.this.currentInputRequestInterval/1000) +" seconds");
				WidgetManager.this.timerInput.schedule( WidgetManager.this.currentInputRequestInterval );
			}
...
private boolean isProcessed(WidgetInput wi) {
	...
	public void removeAllWidgets() {
		...
		this.communicator.deleteAllWidgets(this.placeId, this.applicationId, this.applicationId, null);
